...
class ManyCardsEncoder:
    def __init__(self, seq_dim, profit_dim, price_dim, points_dim, dense1_dim, dense2_dim, l2_penalty, max_points=25):
        ...
        full_cards  = Dense(units=dense1_dim, activation='relu',
                            kernel_regularizer=l2(l2_penalty))(full_cards)
        full_cards = Dense(units=dense2_dim, activation='relu',
                           kernel_regularizer=l2(l2_penalty))(full_cards)
...
class ManyNoblesEncoder:
    def __init__(self, price_dim, dense1_dim, dense2_dim, l2_penalty):
        self.price_encoder = PriceEncoder(output_dim=price_dim)
        ...
        full_nobles = Dense(dense1_dim, activation='relu', kernel_regularizer=l2(l2_penalty))(price_encoded)
        full_nobles = Dense(dense2_dim, activation='relu', kernel_regularizer=l2(l2_penalty))(full_nobles)
...
class BoardEncoder:
    def __init__(self, gems_encoder : GemsEncoder, nobles_encoder: ManyNoblesEncoder, cards_encoder: ManyCardsEncoder,
                 dense_dim1, dense_dim2, l2_penalty):
        ...
        full_board = Dense(dense_dim1, activation='relu', kernel_regularizer=l2(l2_penalty))(full_board)
        full_board = Dense(dense_dim2, activation='relu', kernel_regularizer=l2(l2_penalty))(full_board)
...
class PlayerEncoder:
    def __init__(self, gems_encoder : GemsEncoder, price_encoder : PriceEncoder,
                 reserved_cards_encoder: ManyCardsEncoder, points_dim, nobles_dim, dense_dim1, dense_dim2, l2_penalty):
        ...
        full_player = Dense(dense_dim1, activation='relu', kernel_regularizer=l2(l2_penalty))(full_player)
        full_player = Dense(dense_dim2, activation='relu', kernel_regularizer=l2(l2_penalty))(full_player)
...
@gin.configurable
class StateEvaluatorAveragePooling:
   def __init__(self,
                ...
                network_name: str = 'Average Pooling with L2',
       ...
       self.board_encoder = BoardEncoder(self.gems_encoder,
                                          ...
                                          full_board_dense2_dim, l2_penalty)
       ...
       full_state = Dense(full_player_dense1_dim, activation='relu', kernel_regularizer=l2(l2_penalty))(full_state)
       final_state = Dense(full_player_dense2_dim, activation='relu', kernel_regularizer=l2(l2_penalty))(full_state)
       ...
       self.network = Model(inputs = self.inputs, outputs = result, name = 'Splendor_value_estimator')
